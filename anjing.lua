-- ============================================================
-- WISNU HUB v2.0 (FluentPro Native) -
-- ============================================================
...
Onyx.Callbacks.OnSuccess = function()
    ...
    local Success, Fluent = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/AlDev14/modded-ui/refs/heads/main/FluentProDeval.lua"))()
    end)
    ...
    local function isSCP(obj)
        -- SCP = Model dengan Humanoid yang BUKAN player character
        if not obj or not obj:IsA("Model") then return false end
        local hum = obj:FindFirstChild("Humanoid")
        if not hum then return false end
        -- Pastikan bukan player character
        local isPlayerChar = false
        for _, p in ipairs(Players:GetPlayers()) do
            if p.Character == obj then isPlayerChar = true; break end
        end
        if isPlayerChar then return false end
        -- Filter berdasarkan nama (SCP/zombie/monster dll)
        local name = obj.Name:lower()
        local validNames = {
            "scp", "zombie", "monster", "infected", "mutant",
            "creature", "beast", "entity", "demon", "wraith"
        }
        for _, n in ipairs(validNames) do
            if name:find(n) then return true end
        end
        -- Fallback: jika ada PrimaryPart dan bukan generator/pallet
        if obj.PrimaryPart and not obj:FindFirstChild("Generator")
            and not obj:FindFirstChild("PalletPoint")
            and obj.Name ~= "Window" then
            -- Cek apakah di dalam folder Map (bukan Players folder)
            local inMap = false
            local cur = obj.Parent
            while cur do
                if cur.Name == "Map" or cur.Name == "NPCs" or cur.Name == "Enemies" then
                    inMap = true; break
                end
                cur = cur.Parent
            end
            if inMap then return true end
        end
        return false
    end
    ...
    local function updateESP()
        ...
        if VD.ESP_Window then
            -- Window = Model yang punya VaultPoint/VaultTrigger di dalamnya
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("Model") then
                    local hasVault = obj:FindFirstChild("VaultPoint")
                        or obj:FindFirstChild("VaultTrigger")
                        or obj:FindFirstChild("VaultPointInUse")
                        or obj:FindFirstChild("VaultPointSlide")
                    if hasVault then
                        local part = obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart")
                        if part then
                            local dist = (part.Position - root.Position).Magnitude
                            if dist <= maxDist then
                                createHighlight(obj, COLORS.Window)
                                createBillboard(obj, "WINDOW", COLORS.Window)
                            end
                        end
                    end
                end
                ...
                if gate.Name == "Gate" and gate:IsA("Model") then
                    ...
                    if part then
                        ...
                        if dist <= maxDist then
                            ...
                            createBillboard(gate, "GATE", COLORS.Gate)
                        end
                    end
                end
            end
    ...
    -- UNLIMITED VAULT
    -- ============================================================
    -- Fast Vault (Unlimited + Fast Animation)
    ...
    local function setupFastVaultAnim(char)
        ...
        FastVaultAnimConn = animator.AnimationPlayed:Connect(function(track)
            ...
            if id ~= VAULT_ANIM_ORIG then return end
    ...
    function EnableUnlimitedVault()
        ...
        UnlimitedVaultConn = CollectionService:GetInstanceAddedSignal("Blocked"):Connect(function(instance)
            if VD.UnlimitedVault then
                CollectionService:RemoveTag(instance, "Blocked")
            end
        end)
        -- Fast animation
        setupFastVaultAnim(LocalPlayer.Character)
        Fluent:Notify({Title="Fast Vault", Content="ON - Vault cepat + tanpa cooldown", Type="Success", Duration=2})
    end
    ...
    function DisableUnlimitedVault()
        ...
        Fluent:Notify({Title="Fast Vault", Content="OFF", Type="Info", Duration=2})
    end

    -- Re-setup saat respawn
    LocalPlayer.CharacterAdded:Connect(function(char)
        if VD.UnlimitedVault then
            task.wait(1)
            setupFastVaultAnim(char)
        end
    end)
    ...
    function StartAutoStalk()
        ...
        if not StalkRemote then
            Fluent:Notify({Title="Auto Stalk", Content="Remote tidak ditemukan", Type="Error", Duration=3})
            return
        end
    ...
    -- Veil remote: coba beberapa path (game bisa update struktur)
    local VeilRemote = nil
    pcall(function()
        local remotes = ReplicatedStorage:WaitForChild("Remotes", 5)
        if not remotes then return end
        -- path 1: Remotes.Items.Veil.Activate
        local items = remotes:FindFirstChild("Items")
        local veilFolder = items and items:FindFirstChild("Veil")
        VeilRemote = veilFolder and (
            veilFolder:FindFirstChild("Activate") or
            veilFolder:FindFirstChild("Fire") or
            veilFolder:FindFirstChild("Use") or
            veilFolder:FindFirstChildWhichIsA("RemoteEvent") or
            veilFolder:FindFirstChildWhichIsA("RemoteFunction")
        )
        -- path 2: Remotes.Killers.Veil.*
        if not VeilRemote then
            local killers = remotes:FindFirstChild("Killers")
            local veilK = killers and killers:FindFirstChild("Veil")
            VeilRemote = veilK and (
                veilK:FindFirstChild("Activate") or
                veilK:FindFirstChild("Attack") or
                veilK:FindFirstChildWhichIsA("RemoteEvent")
            )
        end
    end)
    ...
    -- FOV Circle untuk Veil Silent Aim
    local VeilFOVCircle = Drawing.new("Circle")
    ...
    UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if input.KeyCode == Enum.KeyCode.K then
            ...
            Fluent:Notify({Title="Silent Aim", Content="Target: Killer", Type="Info", Duration=2})
        elseif input.KeyCode == Enum.KeyCode.J then
            VD.Pistol_Target = "Survivor"
            Fluent:Notify({Title="Silent Aim", Content="Target: Survivor", Type="Info", Duration=2})
        elseif input.KeyCode == Enum.KeyCode.L then
            VD.Pistol_Target = "SCP"
            Fluent:Notify({Title="Silent Aim", Content="Target: SCP", Type="Info", Duration=2})
        end
        pcall(function()
        ...
        if VD.Pistol_SilentAim and isChargingPistol and AttackAimMode == "Spear" then
            ...
            if target then
                ...
                if myHRP then
                    ...
                    local distance = (targetPos - startPos).Magnitude
                    local time = distance / VD.SpearSpeed
                    local drop = 0.5 * VD.SpearGravity * time * time
    ...
    -- GetActionTarget W424 style
    local function GetActionTargetW424()
    ...
    local function TriggerSkillCheckW424()
        local btn = GetActionTargetW424()
        if btn and btn:IsA("GuiObject") and btn.Visible then
            ...
            pcall(function()
                ...
                VirtualInputManager:SendTouchEvent(tid,0,p.X+s.X/2+i.X,p.Y+s.Y/2+i.Y)
                task.wait(0.005)
                VirtualInputManager:SendTouchEvent(tid,2,p.X+s.X/2+i.X,p.Y+s.Y/2+i.Y)
            end)
            return
        end
        pressSpace()
    ...
    local _ActionPath40 = "Survivor-mob.Controls.action.check"
    local function GetActionTarget40()
        local cur = PlayerGui
        for seg in string.gmatch(_ActionPath40, "[^%.]+") do
            cur = cur and cur:FindFirstChild(seg)
        end
        return cur
    end
    local function TriggerButton40()
        local b = GetActionTarget40()
        if b and b:IsA("GuiObject") then
            if type(firesignal) == "function" then
                pcall(function()
    ...
    -- Parry Range Circle visual
    local ParryCirclePart = nil
    local function updateParryCircle()
        if not VD.Surv_AutoParry then
            if ParryCirclePart then ParryCirclePart:Destroy(); ParryCirclePart = nil end
            return
        end
        ...
        ParryCirclePart.CFrame = CFrame.new(root.Position - Vector3.new(0, yOff, 0))
            * CFrame.Angles(0, 0, math.rad(90))
    end

    -- Update circle tiap Heartbeat
    RunService.Heartbeat:Connect(function()
        if VD.Surv_AutoParry then
            pcall(updateParryCircle)
            ...
            if VD.Surv_ParryAggressive then
                if dist <= 12 then
                    ...
                    tracker = RunService.Heartbeat:Connect(function()
                        if os.clock() - startTime >= 1.5 or ParryState.ParryCooldown or not myHRP or not kHRP or IsDowned(myChar) then
    ...
    local function applyVisualSettings()
        ...
        if VD.ReduceMap then
            -- setfpscap jika tersedia di executor
            pcall(function() setfpscap(30) end)
            -- Rendering quality terendah
            pcall(function()
                settings().Rendering.QualityLevel = Enum.QualityLevel.Level01
                settings().Rendering.EagerBulkExecution = true
            end)
            -- Matikan shadow global
            Lighting.GlobalShadows = false
            Lighting.ShadowSoftness = 0
            -- Batch process descendants agar tidak freeze
    ...
    local function updateCounterVisibility()
        if FPSFrame then
            -- destroy saat OFF (bukan sekadar Enabled=false)
            if not VD.ShowFPS then
                FPSFrame:Destroy()
                FPSFrame = nil
                FPSLabel = nil
                PingLabel = nil
    ...
    local function updateFPS()
        ...
        if now - fpsTime >= 1 then
            ...
            if FPSLabel then
                ...
                FPSLabel.TextColor3 = fps >= 55 and Color3.fromRGB(0,255,0)
    ...
    local function updatePing()
        ...
        if ping == 0 then pcall(function() ping = Stats:GetAveragePing() end) end
        if PingLabel then
    ...
    RunService.Heartbeat:Connect(function()
        if VD.ShowFPS then updateFPS(); updatePing() end
    end)

    -- ============================================================
    -- SKIP CUTSCENE
    -- ============================================================
    local VD_SkipCutscene     = false
    local VD_SkipCutsceneLoad = false
    local _skipCutsceneConn   = nil
    local _skipCutsceneLoadConn = nil

    local function setupSkipCutscene()
        -- Skip cutscene: destroy/hide ScreenGui yang bernama cutscene
        if _skipCutsceneConn then _skipCutsceneConn:Disconnect() end
        _skipCutsceneConn = PlayerGui.ChildAdded:Connect(function(child)
            if not VD_SkipCutscene then return end
            local n = child.Name:lower()
            if n:find("cutscene") or n:find("cinematic") or n:find("intro") or n:find("loading") then
                task.wait(0.1)
    ...
    local function setupSkipCutsceneLoad()
        -- Skip loading screen: fire remote atau destroy loading gui
        if _skipCutsceneLoadConn then _skipCutsceneLoadConn:Disconnect() end
        _skipCutsceneLoadConn = PlayerGui.ChildAdded:Connect(function(child)
            if not VD_SkipCutsceneLoad then return end
            local n = child.Name:lower()
        ...
        -- Coba fire SkipCutscene remote jika ada
        pcall(function()
            local remotes = ReplicatedStorage:FindFirstChild("Remotes")
            if not remotes then return end
            local skip = remotes:FindFirstChild("SkipCutscene", true)
                or remotes:FindFirstChild("SkipIntro", true)
                or remotes:FindFirstChild("CutsceneSkip", true)
            if skip then
                if skip:IsA("RemoteEvent") then skip:FireServer()
                elseif skip:IsA("RemoteFunction") then skip:InvokeServer() end
            end
        end)
    end
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if input.KeyCode == VD.GenBypass_Hotkey and GenBypass.Enabled then
            ...
            if bestPoint and bestDist <= 8 then
                ...
                Fluent:Notify({Title="Gen Bypass", Content="Tidak ada generator dalam jangkauan", Type="Warning", Duration=2})
            end
        end
    end)
    ...
    local function createMoonwalkGui()
        ...
        keybindLbl.Text = "KB: " .. tostring(MOON_CONFIG.KEYBIND)
    ...
    local IgnoreSkillsList = {"IsPhasing","IsBlinking","IsTeleporting","IsCharging","IsCloaked","IsWraith"}
    local MaskNames = {
        Trapper="TRAPPER", Wraith="WRAITH", Hillbilly="HILLBILLY", Nurse="NURSE",
        Shape="SHAPE", Hag="HAG", Doctor="DOCTOR", Huntress="HUNTRESS",
        Cannibal="CANNIBAL", Nightmare="NIGHTMARE", Pig="PIG", Clown="CLOWN",
        Spirit="SPIRIT", Legion="LEGION", Plague="PLAGUE", GhostFace="GHOSTFACE",
        Blight="BLIGHT", Twins="TWINS", Trickster="TRICKSTER", Nemesis="NEMESIS",
        Knight="KNIGHT", Xenomorph="XENO", Chucky="CHUCKY", Unknown="UNKNOWN",
    }
    ...
    local function ShowMaskGui()
        if not CheckIsMasked() then
            Fluent:Notify({Title="Mask Selection", Content="You are not playing The Masked", Type="Warning", Duration=4})
            return
        end
        BuildMaskGui()
        Fluent:Notify({Title="Mask Selection", Content="GUI opened", Type="Success", Duration=2})
    end
    ...
    local function HideMaskGui()
        ...
        Fluent:Notify({Title="Mask Selection", Content="GUI closed", Type="Info", Duration=2})
    end
    ...
    secMiscSurv:AddToggle("SkipCutscene", {
        ...
        Tooltip = "Auto skip/hide cutscene saat match dimulai",
        Callback = function(v)
            VD.SkipCutscene = v
            VD_SkipCutscene = v
            if v then setupSkipCutscene() else
                if _skipCutsceneConn then _skipCutsceneConn:Disconnect(); _skipCutsceneConn = nil end
            end
        end
    })
    secMiscSurv:AddToggle("SkipCutsceneLoad", {
        Title = "Skip Loading Screen",
        Default = VD.SkipCutsceneLoad,
        Tooltip = "Auto skip loading screen / intro",
        Callback = function(v)
            VD.SkipCutsceneLoad = v
            VD_SkipCutsceneLoad = v
            if v then setupSkipCutsceneLoad() else
                if _skipCutsceneLoadConn then _skipCutsceneLoadConn:Disconnect(); _skipCutsceneLoadConn = nil end
            end
        end
    })
    secMiscSurv:AddToggle("AutoCrouch", {
        ...
        Tooltip = "Auto tekan LeftCtrl saat killer Abyssal menyerang (deteksi dari animasi ID)",
        Callback = function(v)
            VD.Surv_AutoCrouch = v
            Fluent:Notify({
                Title = "Auto Dodge",
                Content = v and "ON - auto crouch saat killer attack" or "OFF",
    ...
    secMiscSurv:AddToggle("UnlimitedVault", {
        ...
        Tooltip = "Vault lebih cepat + tanpa cooldown",
        Callback = function(v)
            VD.UnlimitedVault = v
    ...
    secMiscSurv:AddToggle("GenBypassToggle", {
        ...
        Callback = function(v)
            ...
            setGenBypass(v)
            if v then
                Fluent:Notify({Title="Gen Bypass", Content="Aktif - Klik tombol BYPASS di layar atau tekan G", Type="Success", Duration=3})
            else
                Fluent:Notify({Title="Gen Bypass", Content="Nonaktif", Type="Info", Duration=2})
            end
        end
    })
    secMiscSurv:AddToggle("Moonwalk_Enable", {
        Title = "Moonwalk V3 (WisnuVip)",
        Default = VD.Moonwalk_Enabled,
        Tooltip = "Gerakan samping-mundur halus. GUI muncul saat toggle ON ? tekan G untuk toggle di layar.",
        Callback = function(v)
    ...
    secESP:AddButton({
        ...
        Callback = function() updateESP(); Fluent:Notify({Title="ESP", Content="Refreshed", Type="Success", Duration=2}) end
    })
    ...
    secKiller:AddParagraph({ Title = "Spear Aim Settings", Content = "" })
    ...
    local secSpear = tabKiller:AddSection("Spear & Veil Aim", "solar/target-bold")
    ...
    secSpear:AddToggle("SA_Veil", {
        Title = "Silent Aim Veil",
        Default = VD.Veil_SilentAim,
        Callback = function(v)
            VD.Veil_SilentAim = v
            if VeilFOVCircle then VeilFOVCircle.Visible = v and VD.Veil_ShowFOV end
            Fluent:Notify({Title="Silent Aim Veil", Content=v and "ON" or "OFF",
                Type=v and "Success" or "Info", Duration=2})
        end
    })
    secSpear:AddToggle("Veil_ShowFOV", {
        Title = "Veil Show FOV Circle",
        Default = false,
        Callback = function(v)
            VD.Veil_ShowFOV = v
            if VeilFOVCircle then VeilFOVCircle.Visible = v and VD.Veil_SilentAim end
        end
    })
    secSpear:AddSlider("Veil_FOV", {
        Title = "Veil FOV Radius",
        Min = 30, Max = 500, Default = 200, Rounding = 5,
        Callback = function(v)
            VD.Veil_FOV = v
            if VeilFOVCircle then VeilFOVCircle.Radius = v end
        end
    })
    ...
    secDouble:AddToggle("DoubleDamage", {
        ...
        Callback = function(v)
            ...
            if v then
                ...
                Fluent:Notify({Title="Double Damage", Content="Aktif", Type="Success", Duration=2})
            else
                Fluent:Notify({Title="Double Damage", Content="Nonaktif", Type="Info", Duration=2})
            end
        end
    })
    ...
    secHitbox:AddToggle("HitboxExpander", {
        ...
        Callback = function(v)
            ...
            Fluent:Notify({Title="Hitbox Expander", Content=v and "ON ? Size: "..VD.HitboxSize or "OFF", Type="Info", Duration=2})
        end
    })
    ...
    -- ============================================================
    local secPistol = tabAim:AddSection("Silent Aim ToF", "solar/crosshair-bold")
    secPistol:AddToggle("SA_Pistol", {
        Title = "Silent Aim ToF",
    ...
    local function saveOriginalAvatar()
        if _originalDesc then return end
        pcall(function()
            local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
            if hum then _originalDesc = Players:GetHumanoidDescriptionFromUserId(LocalPlayer.UserId) end
        end)
    end
    saveOriginalAvatar()
    ...
    local function resolveUserId(input)
        ...
        if input:match("^%d+$") then return tonumber(input), nil end
        local ok, id = pcall(function() return Players:GetUserIdFromNameAsync(input) end)
        if ok and id and id > 0 then return id, nil end
        return nil, "Username tidak ditemukan: "..input
    end

    local function applyAvatarManual(userId)
        local char = LocalPlayer.Character
        if not char then task.wait(0.5); char = LocalPlayer.Character end
        if not char then return false, "Character tidak ada" end

        -- Method 1: ApplyDescription (executor advanced)
        ...
        if ok1 and desc then
            ...
            if hum then
                ...
                if ok2 then return true, "ApplyDescription" end
            end
        end
        ...
        if not ok3 or not info then return false, "Gagal fetch info avatar" end

        -- 1. Apply body colors DULU (sebelum hapus apapun)
        local bodyColors = char:FindFirstChildOfClass("BodyColors")
        if bodyColors and info.bodyColors then
            pcall(function()
                local bc = info.bodyColors
                if bc.headColor3        then bodyColors.HeadColor3      = bc.headColor3        end
                if bc.torsoColor3       then bodyColors.TorsoColor3     = bc.torsoColor3       end
                if bc.leftArmColor3     then bodyColors.LeftArmColor3   = bc.leftArmColor3     end
                if bc.rightArmColor3    then bodyColors.RightArmColor3  = bc.rightArmColor3    end
                if bc.leftLegColor3     then bodyColors.LeftLegColor3   = bc.leftLegColor3     end
                if bc.rightLegColor3    then bodyColors.RightLegColor3  = bc.rightLegColor3    end
            end)
        end

        -- 2. Collect aset baru dulu, BARU swap
        local newShirt, newPants, newTshirt, newFaceId = nil, nil, nil, nil
        ...
        for _, asset in ipairs(info.assets or {}) do
            ...
            if atype == "Shirt"            then newShirt   = "rbxassetid://"..id
            elseif atype == "Pants"        then newPants   = "rbxassetid://"..id
            elseif atype == "TShirt" or atype == "T-Shirt" then newTshirt = "rbxassetid://"..id
            elseif atype == "Face"         then newFaceId  = "rbxassetid://"..id
            elseif atype:find("Accessory") or atype == "Hat"
                or atype:find("Hair")  or atype:find("Face")
                or atype:find("Neck")  or atype:find("Shoulder")
                or atype:find("Waist") or atype:find("Front")
                or atype:find("Back") then
                table.insert(newAccessories, asset.id)
            end
        end

        -- 3. Ganti Shirt (hapus lama, buat baru)
        local oldShirt = char:FindFirstChildOfClass("Shirt")
        if newShirt then
            if oldShirt then pcall(function() oldShirt:Destroy() end) end
            pcall(function()
                local s = Instance.new("Shirt")
                s.ShirtTemplate = newShirt
                s.Parent = char
            end)
        elseif oldShirt then
            pcall(function() oldShirt:Destroy() end)
        end

        -- 4. Ganti Pants
        local oldPants = char:FindFirstChildOfClass("Pants")
        if newPants then
            if oldPants then pcall(function() oldPants:Destroy() end) end
            pcall(function()
                local p = Instance.new("Pants")
                p.PantsTemplate = newPants
                p.Parent = char
            end)
        elseif oldPants then
            pcall(function() oldPants:Destroy() end)
        end

        -- 5. TShirt
        if newTshirt then
            local oldG = char:FindFirstChildOfClass("ShirtGraphic")
            if oldG then pcall(function() oldG:Destroy() end) end
            pcall(function()
                local g = Instance.new("ShirtGraphic")
                g.Graphic = newTshirt
                g.Parent = char
            end)
        end
        ...
        if newFaceId then
            ...
            if face and face:IsA("Decal") then
                pcall(function() face.Texture = newFaceId end)
            end
        end

        -- 7. Hapus accessories lama
        for _, child in ipairs(char:GetChildren()) do
            if child:IsA("Accessory") then
                pcall(function() child:Destroy() end)
            end
        end
        -- Load accessories baru via game:GetObjects (Delta-safe)
        for _, assetId in ipairs(newAccessories) do
            pcall(function()
                local objs = game:GetObjects("rbxassetid://"..tostring(assetId))
                for _, obj in ipairs(objs or {}) do
                    if obj:IsA("Accessory") then
                        obj.Parent = char
                        -- Weld handle ke HRP agar tidak jatuh
        ...
        return true, "Manual (Shirt/Pants/Face/Acc)"
    end

    local function applyAvatarByInput(input)
        input = tostring(input or ""):gsub("^%s+",""):gsub("%s+$","")
        if input == "" then
            Fluent:Notify({Title="Avatar Copy", Content="Input username/ID kosong!", Type="Warning", Duration=3})
            return
        end
        task.spawn(function()
            Fluent:Notify({Title="Avatar Copy", Content="Memuat avatar...", Type="Info", Duration=2})
            local userId, err = resolveUserId(input)
            if not userId then
                Fluent:Notify({Title="Avatar Copy", Content=err, Type="Error", Duration=4})
                return
            end
            local ok, method = applyAvatarManual(userId)
            if ok then
                Fluent:Notify({
                    Title   = "Avatar Copy",
                    Content = "Berhasil via "..method.." (client-side only)",
                    Type    = "Success", Duration = 3
                })
            else
                Fluent:Notify({
                    Title   = "Avatar Copy",
                    Content = "Gagal: "..tostring(method),
    ...
    local function restoreMyAvatar()
        ...
        if _originalDesc then
            pcall(function() hum:ApplyDescription(_originalDesc) end)
            Fluent:Notify({Title="Avatar Copy", Content="Avatar dikembalikan ke asli", Type="Info", Duration=3})
        end
    end
    ...
    -- Avatar Copy UI
    local secAvatar = tabMisc:AddSection("Avatar Copy", "solar/user-id-bold")
    local _avatarInput = ""
    secAvatar:AddInput("AvatarInput", {
        Title = "Username / User ID",
        Placeholder = "e.g. Roblox or 1",
        Callback = function(v) _avatarInput = v end,
        Finished = true,
    })
    secAvatar:AddButton({
        Title = "Apply Avatar",
        Callback = function()
            applyAvatarByInput(_avatarInput)
        end
    })
    secAvatar:AddButton({
    ...
    secAvatar:AddParagraph({
        Title = "Info",
        Content = "Client-side only. Hanya kamu yang melihat perubahan. Target tidak perlu di server yang sama."
    })

    -- ============================================================
    -- UI SETTINGS
    -- ============================================================
    Fluent.MediaManager:SetFolder("WisnuHub/MediaCache")
    Fluent.InterfaceManager:SetLibrary(Fluent)
    Fluent.InterfaceManager:SetFolder("WisnuHub/Interface")
    Fluent.InterfaceManager:BuildInterfaceSection(tabUI)
    Fluent.InterfaceManager:LoadSettings()

    Fluent.SaveManager:SetLibrary(Fluent)
    Fluent.SaveManager:SetFolder("WisnuHub/Config")
    Fluent.SaveManager:IgnoreThemeSettings()
    Fluent.SaveManager:BuildConfigSection(tabUI)
